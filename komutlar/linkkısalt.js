...
exports.run = (client, message, args, tools) => {
  ...
  if (!args[0]) return message.channel.send('**Doğru Kullanım : l-linkkısalt <URL> [yazı]**')
  ...
  if (!args[1]) { // If the second argument in the message is undefined, run this
    ...
    shorten.shorten(args[0], function(res) { // This will run the shorten function and pass it 'res'
      if (res.startsWith('Error:')) return message.channel.send('**Lütfen geçerli bir link verin.**'); // The only possible error here would be that it's not a valid URL.
